...
const CategoryPlaylistsCollection: React.FC<CategoryPlaylistsCollectionProps> = ({ id, name, lengthToDisplay }) => {
    ...
    const getPlaylists = async (accessToken: string) => {
        const { playlists: { items } } = await getCategorysPlaylists(id, accessToken, lengthToDisplay);
    ...
    return (
        <section>
            <div className="flex justify-between items-center mb-4">
                <Link to={`/category/${id}`} className="text-2xl font-bold hover:underline">{name}</Link>
            ...
            >
                {playlists && playlists.slice(0, lengthToDisplay).map(playlist => (
                    <PlaylistCard
                        key={playlist.id}
                        id={playlist.id}
